...
template <typename Iterator,
          typename UnaryFunc>
std::vector<typename Iterator::value_type> makeVec(Iterator first, Iterator last, UnaryFunc f) {
...
template <typename Iterator,
          typename UnaryFunc>
std::vector<typename Iterator::value_type> imakeVec(Iterator first, Iterator last, UnaryFunc f) { // i for iterator, -- f's parameter is iterator
    // std::vector<std::decay_t<decltype(f(first))>> res;
    std::vector<typename Iterator::value_type> res;
    for (; first != last; ++first) res.push_back(f(first));
    return res;
...
template <typename Container, typename T = typename Container::value_type,
          typename UnaryFunc>
inline std::vector<typename Container::value_type> makeVec(const Container &vec, UnaryFunc f) {
    return makeVec(std::begin(vec), std::end(vec), f);
}
template <typename T,
          typename UnaryFunc>
inline std::vector<T> makeVec(std::initializer_list<T> c, UnaryFunc f) {
...
template <typename Container>
std::map<typename Container::value_type, int> Rank(const Container &c) {
    return Rank(std::begin(c), std::end(c));
}
template <typename T>
std::map<T, int> Rank(std::initializer_list<T> c) {
    return Rank(std::begin(c), std::end(c));
}
...
bool is_sorted(const Container &c, Compare comp = Compare()) { return std::is_sorted(std::begin(c), std::end(c), comp); }
template <typename T, typename Compare = std::less<T>>
bool is_sorted(std::initializer_list<T> c, Compare comp = Compare()) { return std::is_sorted(std::begin(c), std::end(c), comp); }
