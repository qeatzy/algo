#include "utility.h"
// "tp(two pointer), see also " algo
// 1. crossing, eg, Hoare partition. 2. sliding window. 3. slow-fast, eg, floyd cycle detection.
// 4. shrink/expand array, eg, delete all space in string, replace space with "%20" assuming enough memory at the end.
// -- eg, remove duplicates from sorted array -- remove adjacent duplicate -- std::unique.
...
// Given input array nums = [1,1,2], Your function should return length = 2.
    int removeDuplicates(vector<int>& nums) {
        ...
        auto d = nums.begin(), last = nums.end();
        if (d == last) return 0;
        auto p = d;
        for (++p; p != last && *d != *p; ++d, ++p) {}
        if (p != last)
            for (;;) {
                for (auto tmp = p; ++p != last && *p == *tmp; ) {}
                if (p == last) break;
                *++d = *p;
            }
        return 1 + (d - nums.begin());
    }

template <typename Iterator, typename T = typename std::iterator_traits<Iterator>::value_type>
 Iterator unique(Iterator first, Iterator last) { return first; }
...
namespace test {
    void removeDuplicates() {
        ...
        for (auto &v: vec) {
            ...
            cout << "v.size() = " << v.size() << ", after remove duplicates, sz = " << res << endl;
        }
    }
}
