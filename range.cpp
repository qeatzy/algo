#include "utility.h"
// range, partial sum or aggregation of range, see also " binarySearch.cpp " binaryIndexTree.cpp " segmentTree.cpp
// Partial sum and more data structure.  http://codeforces.com/blog/entry/15729
// Can please someone explain Partial sum part 2? What is the problem it solve?  Partial Sum part 2 solves the problem where you are given an array and all you have to do is to print the array after performing q queries where each query asks you to change all the elements between [l,r]. Say you have an array of size of the order 10^5 and you have 10^5 queries. Now Naive method is to iterate over all the r-l+1 elements for each query hence the solution would be O(n^2). So according to part 2 of the solution all you do is to update two elements for each query and O(n) in the end. So the solution would be O(n).  http://codeforces.com/blog/entry/15729
...
// fixed bug, add extra 0, use inclusive or left-inclusive-right-exclusive properly and consistently.
// fixed bug, partial_sum overflow error.
// -- similar idea with countSmaller in binaryIndexTree.cpp or segmentTree.cpp
// -- maintain seg tree or BIT which counts only candidates, and query specific range accordingly.
// -- but here is also decrement, beside increment.
    int countRangeSum(vector<int>& nums, int lower, int upper) {
        int res = 0;
        const int N = nums.size() + 1;
        auto psum = std::vector<long long> (N);
        std::copy(nums.begin(), nums.end(), psum.begin() + 1);
        std::partial_sum(psum.begin() + 1, psum.end(), psum.begin() + 1);
        auto pos = range(N);
        std::stable_sort(pos.begin(), pos.end(), [&psum](int i, int j) { return psum[i] < psum[j]; });
        // print(psum,"psum");
        // print(pos,"pos");
        ...
        for (int i = 0; i < N; ++i) {
            if (pos[i] == 0) continue;
            // add candidates
            for (; index < N && psum[pos[index]] <= psum[pos[i]] - lower; ++index) {
                int t = N + pos[index];
                for (++seg[t]; t > 1; t >>= 1) seg[t>>1] = seg[t] + seg[t^1];
            }
            // remove obsolete candidates
            for (; r_index < index && psum[pos[r_index]] < psum[pos[i]] - upper; ++r_index) {
                int t = N + pos[r_index];
                for (--seg[t]; t > 1; t >>= 1) seg[t>>1] = seg[t] + seg[t^1];
            }
            // query
            for (int l = N, r = N + pos[i]; l < r; l >>= 1, r >>= 1) {
                if (l&1) res += seg[l++];
                if (r&1) res += seg[--r];
            }
            // cout << "pos[i] = " << pos[i] << ", psum[pos[i]] = " << psum[pos[i]] << ", res = " << res << endl;
        }
        return res;
    }
...
namespace test {
    void countRangeSum() {
        ...
        vec.push_back({0,  0,0});
        vec.push_back({-2,5,-1,  -2,2});
        vec.push_back({-2147483647,0,-2147483647,2147483647,   -564,3864});
        vec.push_back({2147483647,-2147483648,-1,0,   -1, 0});
        for (auto &v: vec) {
            assert(v.size() > 2);
            int upper = v.back(); v.pop_back();
            int lower = v.back(); v.pop_back();
            print(v,"v");
            auto res = ::countRangeSum(v, lower, upper);
            cout << res << " ranges in v such that belows to [" << lower << ", " << upper << "] " << endl;
        }
    }
}
...
class NumArray {
public:
    NumArray(vector<int> nums) : psum(nums.size() + 1), values(nums.size()) {
...
class NumArray_immutable{
public:
    NumArray_immutable(vector<int> nums) : prefix_sum(nums.size() + 1) {
...
namespace test {
    void sumRange() {
        ...
        auto obj = ::NumArray_immutable(nums);
